...
public class BlockCompactSolar extends BlockContainer {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
        ...
        if (te != null && te instanceof TileEntityCompactSolar) {
            ...
            playerIn.openGui(CompactSolars.instance, tecs.getType().ordinal(), worldIn, pos.getX(), pos.getY(),
                    pos.getZ());
        }
        return true;
    }
    ...
    public void dropContent(int newSize, TileEntityCompactSolar tileSolar, World world) {
        for (int l = newSize; l < tileSolar.getSizeInventory(); l++) {
            ...
            while (itemstack.stackSize > 0) {
                ...
                EntityItem entityitem = new EntityItem(world, tileSolar.getPos().getX() + f,
                        (float) tileSolar.getPos().getY() + (newSize > 0 ? 1 : 0) + f1, tileSolar.getPos().getZ() + f2,
